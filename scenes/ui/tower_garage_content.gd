...
## shell, which owns the shared NavBar and calls `set_active()`/`_refresh()` on
## this node directly.
...
## Called by the shell whenever this screen becomes (or stops being) the one
## on screen — pauses the 3D preview's continuous render while off-screen so a
## garage the player isn't looking at doesn't keep paying full 3D render cost.
func set_active(active: bool) -> void:
	preview.set_rendering_active(active)
