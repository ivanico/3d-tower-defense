...
## DISTINCT spell the tower has picked this run (Tower._active_spells, via
## the new get_active_spells() accessor), with a row of gold dots under each
## icon showing how many times that spell's been picked
## (Tower.get_stack_count) — mirrors the draft card's own icon (SpellIcon,
## shared — see spell_icon.gd) and rank-pip row (SpellRankPips, shared) so
## this reuses both instead of re-implementing icon/pip rendering.
##
...
func _rebuild() -> void:
	...
	var counts: Array
	if Engine.is_editor_hint():
		spells = preview_spells
		counts = preview_counts
	else:
		var tower := get_tree().get_first_node_in_group("tower")
		if tower == null:
			return
		spells = tower.get_active_spells()
		counts = spells.map(func(s): return tower.get_stack_count(s.spell_id))
	for i in spells.size():
		...
		# stale copy behind.
		add_child(_build_cell(spell, count), false, Node.INTERNAL_MODE_BACK)

func _build_cell(spell: SpellDefinition, count: int) -> VBoxContainer:
